...
def build_settings_from_config(
    ...
    """Convert YAML data into validated generator settings and load its scene."""
    ...
    experiment_name = _read_experiment_name(config, config_path)
    target_type = require_string(teacher_config, "target_type")
    ...
    teacher = _build_teacher(teacher_config, target_type)
...
def _read_experiment_name(config: dict[str, Any], config_path: Path) -> str:
...
def _read_variant(mitsuba_config: dict[str, Any]) -> str:
    variant = mitsuba_config.get("variant", DEFAULT_VARIANT)

    if not isinstance(variant, str) or variant.strip() == "":
        raise ValueError("mitsuba.variant must be a non-empty string.")

    return variant


def _reject_removed_teacher_options(teacher_config: dict[str, Any]) -> None:
    if "ray_epsilon" in teacher_config:
        raise ValueError(
            "teacher.ray_epsilon was removed because Mitsuba spawn_ray() "
            "already applies the required ray-origin offset."
        )


def _build_camera(camera_config: dict[str, Any]) -> DatasetCamera:
...
def _build_bins(bins_config: dict[str, Any]) -> HemisphereBins:
