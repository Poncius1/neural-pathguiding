...
Teachers estimate the importance of candidate incoming directions.
They do not normalize contributions or write datasets.
...
# T0: heuristic visibility multiplied by the surface cosine.
# This target is useful only as a pipeline sanity check.
VISIBILITY_COSINE_TARGET = "visibility_cosine_v0"

# T1: luminance of incident radiance Li(x, wi).
# This target guides toward incoming light without considering the BSDF.
INCIDENT_RADIANCE_TARGET = "incident_radiance_v1"

# T2: luminance of Li(x, wi) * fs(x, wo, wi), multiplied by cosine.
# This is the principal full-product target of the thesis.
...
# A target becomes supported only after its estimator and tests exist.
...
class TeacherContext:
    """Information shared by all directional teacher evaluations."""
...
class VisibilityCosineTeacherSettings:
    """Configuration for the T0 visibility/cosine teacher."""
...
class PhysicalTeacherSettings:
    """Configuration shared by the T1 and T2 physical teachers.

    samples_per_bin controls how many candidate directions are
    generated inside each directional bin.

    radiance_samples controls how many independent radiance
    estimates are calculated for each candidate direction.

    max_depth limits the paths used to estimate incident radiance.

    rr_depth specifies the depth at which Russian roulette starts.
    """
...
def validate_teacher_target_type(
    ...
    """Reject unknown or not-yet-implemented target types."""
...
def validate_physical_teacher_settings(
    ...
    This function does not mark the target as implemented. It only
    validates the structure of a physical-teacher configuration.
    """
    ...
    if (
        not np.isfinite(teacher.smoothing)
        or teacher.smoothing < 0.0
    ):
        raise ValueError(
...
def validate_visibility_cosine_teacher_settings(
    ...
    """Validate the settings used by the T0 heuristic teacher."""
...
def validate_teacher_settings(teacher: TeacherSettings) -> None:
    """Validate any known teacher configuration.

    Structural pattern matching is appropriate here because these are
    mutually exclusive teacher types. Validation conditions inside each
    type remain simple guard clauses.
    """
    match teacher:
        case VisibilityCosineTeacherSettings():
            validate_visibility_cosine_teacher_settings(teacher)

        case PhysicalTeacherSettings():
            validate_physical_teacher_settings(teacher)
...
def build_teacher_metadata(
    ...
    """Return serializable metadata for any teacher configuration."""
    ...
    match teacher:
        ...
        case _:
            raise TypeError(
                "Cannot build metadata for an unknown teacher type."
            )
...
def estimate_bin_contributions(
    ...
    teacher_estimator = resolve_teacher_estimator(
        teacher.target_type
    )
    ...
    for bin_index in range(bins.n_bins):
        ...
        for _ in range(teacher.samples_per_bin):
            ...
            contribution = teacher_estimator(
                context=context,
...
def estimate_visibility_cosine_contribution(
    ...
    """Evaluate the T0 visibility/cosine sanity-check teacher."""

    normalized_normal = normalize_numpy_vector(
        context.normal
    )
    ...
    ray = context.surface_interaction.spawn_ray(
        _mi.Vector3f(*normalized_direction)
    )
...
def surface_hit_has_emitter(
    ...
    """Return whether a surface interaction belongs to an emitter."""
...
def resolve_teacher_estimator(
    ...
    """Return the estimator associated with a target type."""
